...
def generate_samples(raw_data_member, raw_data_non_member, batch_size):
    ...
    effective_n = min(len(raw_data_member), len(raw_data_non_member), args.n_samples)

    if effective_n < args.n_samples:
        raise ValueError(
            f"Not enough paired samples. Need {args.n_samples}, got {effective_n}. "
            f"member={len(raw_data_member)}, nonmember={len(raw_data_non_member)}"
        )

    raw_data_member = raw_data_member[:effective_n]
    raw_data_non_member = raw_data_non_member[:effective_n]

    seq_lens = []
    for batch in range(effective_n // batch_size):
...
def generate_data(dataset, key, train=True):
    """
    Robust dataset loader for true n-sample MIA evaluation.

    For CNN/DailyMail:
      member     = train split
      non-member = validation split

    This version avoids the old adaptive >100-word filter that caused
    validation to shrink to only 373 samples.
    """
    if dataset in custom_datasets.DATASETS:
        data = custom_datasets.load(dataset, cache_dir)

    elif dataset in ["cnn_dailymail", "cnn_dailymail_highlights", "xsum"]:
        if dataset == "xsum":
            split = "train[:50000]" if train else "validation"
            real_key = "document" if key in [None, ""] else key
            print(f"[Dataset] Loading xsum split={split}, field={real_key}")
            ...
            split = "train[:50000]" if train else "validation"
            real_key = "highlights" if key in [None, "", "document"] else key
            print(
                f"[Dataset] Loading cnn_dailymail v3.0.0 "
        ...
        real_key = "text" if key in [None, ""] else key
        print(f"[Dataset] Loading ag_news split={split}, field={real_key}")
        data = datasets.load_dataset(
            "ag_news",
            split=split,
            cache_dir=cache_dir,
        ...
        data_split = "train[:50000]" if train else "test"
        print(f"[Dataset] Loading {dataset} split={data_split}, field={key}")
        data = datasets.load_dataset(
            dataset,
            split=data_split,
            cache_dir=cache_dir,
    ...
    data = [strip_newlines(x.strip()) for x in data if x.strip()]

    # Keep not-too-short samples, but do NOT force >100 words.
    # CNN/DailyMail highlights are often around 40-60 words.
    min_words = 20
    data = [x for x in data if len(x.split()) >= min_words]

    random.seed(0)
    random.shuffle(data)

    # Keep examples that T5 can perturb safely.
    # Do it in chunks so we do not tokenize the whole dataset unnecessarily.
    target_keep = max(args.n_samples, 5000)
    kept = []

    for i in range(0, len(data), 1000):
        batch = data[i:i + 1000]
        tokenized = preproc_tokenizer(batch, truncation=False, padding=False)
        for x, y in zip(batch, tokenized["input_ids"]):
            if len(y) <= 512:
                kept.append(x)
    ...
    if len(data) < args.n_samples:
        raise ValueError(
            f"Not enough usable samples for dataset={dataset}. "
            f"Need {args.n_samples}, got {len(data)}. "
            f"Try a larger split or a different dataset."
        )
