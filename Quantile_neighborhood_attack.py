...
def _clean_ll_array(values):
    arr = np.array(values, dtype=float)
    arr = arr[np.isfinite(arr)]
    return arr


def _empirical_quantile_score(x_ll, neighbor_lls):
    """
    QN-MIA score.

    Higher score means the original text has higher likelihood than more of its neighbors.

        score = (1 + # { z in N(x) : LL(z) <= LL(x) }) / (|N(x)| + 1)

    Range:
        near 1   -> original is unusually high-likelihood compared to neighbors
        near 0   -> original is not special compared to neighbors
    """
    if not np.isfinite(x_ll):
        return float("nan")

    neigh = _clean_ll_array(neighbor_lls)

    if len(neigh) == 0:
        return float("nan")

    return float((1.0 + np.sum(neigh <= x_ll)) / (len(neigh) + 1.0))


def run_qn_experiment(results, span_length, n_perturbations, n_samples):
    """
    Quantile Neighborhood MIA.

    For nonmember/original:
        QN_original = rank_quantile(original_ll, all_perturbed_original_ll)

    For member/sampled:
        QN_member = rank_quantile(sampled_ll, all_perturbed_sampled_ll)
    ...
    for res in results:
        original_score = _empirical_quantile_score(
            res["original_ll"],
            res["all_perturbed_original_ll"],
        )

        sampled_score = _empirical_quantile_score(
            res["sampled_ll"],
            res["all_perturbed_sampled_ll"],
        )

        predictions["real"].append(original_score)
        predictions["samples"].append(sampled_score)

        res["original_qn_score"] = original_score
        res["sampled_qn_score"] = sampled_score

    fpr, tpr, roc_auc = rm.get_roc_metrics(
        predictions["real"],
        predictions["samples"],
    )
    ...
    name = f"QN_MIA_n{n_perturbations}"
...
# def run_attack(args: SimpleNamespace):
#     """
#     Run RN-MIA only if criterion='rn'.

#     For RN-MIA:
#     - patch rm.get_ll to return target-reference log-likelihood
#     - use the existing internal criterion 'd'
#     - rename output as RN_MIA
#     """
#     outputs = []
...
#     print(f"Saved RN-MIA results to {args.save_path}")

def run_attack(args: SimpleNamespace):
    """
    Run selected neighborhood attack.

    qn:
        Quantile Neighborhood MIA.
        Uses empirical rank/quantile of the original LL among neighbor LLs.

    rn:
        Residual Neighborhood MIA.
        Uses target-reference LL, then internal neighborhood difference d.
    """
    outputs = []
    ...
    print(f"Saved QN-MIA results to {args.save_path}")
...
def parse_cli() -> argparse.Namespace:
    ...
    #     choices=["d", "z", "rn"],
    #     default="rn",
    #     help="Attack criterion. Use rn for RN-MIA.")
    p.add_argument(
        "--criterion",
        type=str,
        choices=["d", "z", "rn", "qn"],
        default="qn",
        help="Attack criterion. Use qn for Quantile Neighborhood MIA.")
